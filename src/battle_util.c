    ...
    case ABILITYEFFECT_ABSORBING: // 3
        if (move != MOVE_NONE)
        {
            ...
            case ABILITY_MAGMA_ARMOR:
                if (moveType == TYPE_WATER && gMovesInfo[move].target != MOVE_TARGET_ALL_BATTLERS)
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        ...
        case ABILITY_VENOM_COATING:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && IsMoveMakingContact(move, gBattlerAttacker)
             && TARGET_TURN_DAMAGED // Need to actually hit the target
             && RandomPercentage(RNG_VENOM_COATING, 20))
            {
...
static inline u32 CalcMoveBasePowerAfterModifiers(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    case ABILITY_STEELWORKER:
        if (moveType == TYPE_STEEL)
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_HONED_MIND:
        if (moveType == TYPE_PSYCHIC || moveType == TYPE_GHOST)
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        break;
